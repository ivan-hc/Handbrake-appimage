...
DEPENDENCES="gst-libav gst-plugins-bad gst-plugins-bad-libs gst-plugins-base gst-plugins-base-libs gst-plugins-espeak gst-plugins-good gst-plugins-ugly gstreamer-vaapi gtk4 handbrake-cli ibus jansson lame libass libbs2b libibus libjpeg-turbo libglvnd librsvg libsndfile libtheora libva libvorbis libvpx libxml2 mesa numactl opus slang speex x264 xz zlib" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="alsa jack pipewire pulse gdk-pixbuf libavahi libcups libcolord libexpat.so libGLdispatch libEGL libGLX libgstplay libidn liblcms libLLVM libsensors.so libSPIRV \
...
libjxl libssh libpcaudio libinstpatch libfftw libsord libOpenEXRCore libshaderc libsratom libglslang-default-resource-limits libdovi"
...
case "$1" in
	'--cli'|'cli'|'--help'|'help'|'-h') _JUNEST_CMD -- /usr/bin/HandBrakeCLI "$@";;
	''|*) _JUNEST_CMD -- $EXEC "$@";;
esac
