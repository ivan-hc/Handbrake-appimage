...
APP=handbrake
BIN="ghb"
DEPENDENCES="ca-certificates libcups ffmpeg \
...
function _create_AppRun() {
	...
	BINDS=" --dev-bind /dev /dev \
		...
		--bind-try /usr/share/icons/hicolor /usr/share/icons/hicolor \
...
BINSAVED="certificates mkdir" # Enter here keywords to find and save in /usr/bin
...
function _enable_mountpoints_for_the_inbuilt_bubblewrap() {
	...
	mkdir -p ./"$APP".AppDir/.junest/usr/share/icons/hicolor
